...
const parseReviewWorker = require('./parse-review.worker');
...
function fetchReviews({name, type, currentPage = 1}) {
    ...
    return axios
        ...
        .then(({data}) => {
            ...
            return parseReviewWorker.parseReviewAsync({data, checkForTotalPages: true});
        })
        ...
        .then((allResponses) => {
            ...
            return Promise.all(allResponses
                .map(({data}) => parseReviewWorker.parseReviewAsync({data, totalPages})));
        })
        .then((results) => {
...
module.exports = ({name, type}) => fetchReviews({name, type}).then((result) => {
    ...
    parseReviewWorker.pool.terminate();
    return result;
}).catch((e) => {
    parseReviewWorker.pool.terminate();
    throw e;
