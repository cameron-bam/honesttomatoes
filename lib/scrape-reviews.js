...
function parseResponse({response: {data}, result, currentPage, ...rest}) {
    ...
    if (currentPage < newResult.totalPages) {
        return fetchReviews({currentPage: currentPage + 1, result: newResult, ...rest});
    }

    return newResult;
}

function fetchReviews({name, type, currentPage = 1, result = {}}) {
    ...
    return axios
        ...
        .then((response) => parseResponse({response, result, currentPage, name, type}))
        .catch(console.error.bind(console));
}

module.exports = ({name, type}) => fetchReviews({name, type}).then((result) => {
    result.rtScore = Math.round(100 * (result.rtFresh / result.reviewCount));
