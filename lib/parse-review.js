...
const getTotalPagesProfiled = profile(function getTotalPages({window: {document}}) {
...
const checkForFreshRottenMisMatchProfiled = profile(function checkForFreshRottenMisMatch({window: {document}}) {
...
const getDomProfiled = profile(function getDom(data) {
    return new JSDOM(data);
});

function parseReview({data, checkForTotalPages = false}) {
    const dom = getDomProfiled(data);
